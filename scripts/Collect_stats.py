...
def gql(query, variables=None):
    r = session.post("https://api.github.com/graphql", json={"query": query, "variables": variables or {}})
    r.raise_for_status()
    j = r.json()
    if "errors" in j:
    ...
    query($org:String!, $cursor:String) {
      organization(login:$org) {
        repositories(first:100, after:$cursor, privacy:PUBLIC, orderBy:{field:UPDATED_AT, direction:DESC}) {
          nodes { name nameWithOwner isArchived isDisabled updatedAt pushedAt
...
def main():
    ...
    for r in repos:
        ...
        repo_info = {
            ...
            "contributors": [{"login": k, "commits": v} for k, v in sorted(by_author.items(), key=lambda x: x[1], reverse=True)],
            "last_commit_at": last_commit_iso or r["pushedAt"],
        }
